...
K <- 10
# Simulate data:
data <- simLNIRT(N = 10, K = 24, rho = 0.9, WL = T)
data <- simLNIRT(N = 500, K = 20, rho = 0.8)
data$Y[1,1]<- data$Y[3,4]<- data$Y[8,24]<- data$Y[1,20]<- data$Y[10,1]<- data$Y[2,3]<- data$Y[8,9] <- NAs


out <- LNRT(RT = data$RT, XG = 1000, WL = T)
out1 <- LNRT(RT = data$RT, XG = 1500)
...
### LNRT ###
##
out1 <- LNRT(RT = RT, data = data, XG = 1500)
summary(out1)
##
out2 <- LNRT(RT = RT, data = data, XG = 1500, WL = T)
summary(out2)
##
out3 <- LNRT(RT = RT, data = data, XG = 1500, Discrimination = F, WL = T)
summary(out3)
##
out4 <- LNRT(RT = RT, data = data, XG = 500, Discrimination = T, WL = F)
summary(out4)
##
out5 <- LNRT(RT = data$RT, XG = 1500)
summary(out5)


### LNIRT ###
##
out1 <- LNIRT(RT = RT1, Y = Y, data = data, XG = 1000, WL = T)
summary(out1)
##
out2 <- LNIRT(RT = RT1, Y = Y, data = data, XG = 1200, guess = T, WL = F, par1 = F)
summary(out2)
##
out3 <- LNIRT(RT = data$RT1, Y = data$Y, XG = 1000, guess = T, WL = F)
summary(out3)
##
out4 <- LNIRT(RT = data$RT1, Y = data$Y, XG = 1500, WL = F, residual = T)
summary(out4)
plot(out4$PFl, out4$PFlp)  #ability
plot(out4$EAPl0[,1], out4$lZP, col = "red")
##
out5 <- LNIRT(RT = RT1, Y = Y, data = data, XG = 1500, WL = T, residual = T)
summary(out5)
...
## Y = response matrix of dim(N=persons,K=items) T = log-response time matrix (time spent on solving an item) of dim(N=persons,K=items) XG =
## number of iterations for the MCMC algorithm guess: optional variable to indicate if guessing parameters should be included in the IRT model.
## Use any number you like, e.g., guess = 1
...
data <- simLNIRT(N = 1000, K = 20, rho = 0.8)
