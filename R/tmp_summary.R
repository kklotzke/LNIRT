...
summaryIRT <- function(out, data) {
    ...
    pSdiscr2 <- matrix(c(round(apply(out$MSI[bi:XG, 1, ], 2, mean), 3), round(apply(out$MSI[bi:XG, 2, ], 2, mean), 3), round(apply(out$MSI[bi:XG, 
        3, ], 2, mean), 3), round(apply(out$MSI[bi:XG, 4, ], 2, mean), 3)), ncol = 4, nrow = 4, byrow = TRUE)
    sepSdiscr2 <- matrix(c(round(sqrt(apply(out$MSI[bi:XG, 1, ], 2, var)), 3), round(sqrt(apply(out$MSI[bi:XG, 2, ], 2, var)), 3), round(sqrt(apply(out$MSI[bi:XG, 
        3, ], 2, var)), 3), round(sqrt(apply(out$MSI[bi:XG, 4, ], 2, var)), 3)), ncol = 4, nrow = 4, byrow = TRUE)
    ...
    pSpers2 <- matrix(c(round(mean(out$MSP[bi:XG, 1, 1]), 3), round(mean(out$MSP[bi:XG, 2, 1]), 3), round(mean(out$MSP[bi:XG, 1, 2]), 3), round(mean(out$MSP[bi:XG, 
        2, 2]), 3)), ncol = 2, nrow = 2)
    sepSpers2 <- matrix(c(round(sqrt(var(out$MSP[bi:XG, 1, 1])), 3), round(sqrt(var(out$MSP[bi:XG, 2, 1])), 3), round(sqrt(var(out$MSP[bi:XG, 
        1, 2])), 3), round(sqrt(var(out$MSP[bi:XG, 2, 2])), 3)), ncol = 2, nrow = 2)
    ...
    if ("lZP" %in% names(out)) 
        {
            ...
            cat("\n\t", "lZ", "\n")
            cat("\t", round(100 * length(which(out$lZP < 0.05))/N, 2), "%", "\t\n")
            cat("\t 95% Posterior Probability: ", round(100 * sum(out$EAPCP1 > 0.95)/N, 2), "%", "\t\n")
            ...
            if (length(set >= 1)) {
                ...
                cat("\t", round(100 * length(which(out$EAPKS < 0.05)))/K, "%", "of items has non-lognormally distributed residuals", "\t\n")
                set <- which(out$EAPKS < 0.05)
                if (length(set) > 0) {
                  cat("\t", "Item", "\t P-value ", "\n")
                ...
                cat("\n\t", "Kolmogorov Smirnov Test (5% level)", "\n")
                cat("\t", round(100 * length(which(out$EAPKS < 0.05)))/K, "%", "\t\n")
                set <- which(out$EAPKS < 0.05)
                if (length(set) > 0) {
                  cat("\t", "Item", "\t P-value ", "\n")
                  for (jj in 1:length(set)) {
            ...
            cat("\t", round(100 * length(which(out$PFlp < 0.05))/N, 2), "%", "\t\n")
            cat("\t 95% Posterior Probability: ", round(100 * sum(out$EAPCP2 > 0.95)/N, 2), "%", "\t\n")
            cat("\t 95% Posterior Probability (Ability and Speed): ", round(100 * sum(out$EAPCP3 > 0.95)/N, 2), "%", "\t\n")
            ...
            if (length(set >= 1)) {
                ...
                cat("\n\t", "Kolmogorov Smirnov Test (5% level)", "\n")
                cat("\t", round(100 * length(which(out$EAPKSA < 0.05)))/K, "%", "of items has non-normally distributed latent residuals", "\t\n")
                set <- which(out$EAPKSA < 0.05)
                if (length(set) > 0) {
                  cat("\t", "Item", "\t P-value ", "\n")
                  for (jj in 1:length(set)) {
                    cat("\t")
                ...
                cat("\t", "No Extreme Residuals", "\t\n")
                cat("\n\t", "Kolmogorov Smirnov Test (5% level)", "\n")
                cat("\t", round(100 * length(which(out$EAPKSA < 0.05)))/K, "%", "\t\n")
                set <- which(out$EAPKSA < 0.05)
                if (length(set) > 0) {
                  cat("\t", "Item", "\t P-value ", "\n")
                  for (jj in 1:length(set)) {
...
summaryRT <- function(out, data) {
    ...
    if ("lZP" %in% names(out)) {
        ...
        cat("\n\t", "Percentage Outliers Persons (5% level)", "\n")
        cat("\n\t", "lZ", "\n")
        cat("\t", round(100 * sum(out$lZP < 0.05)/N, 2), "%", "\t\n")
        cat("\t 95% Posterior Probability: ", round(100 * sum(out$EAPCP > 0.95)/N, 2), "%", "\t\n")
