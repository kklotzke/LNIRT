simLNIRT <- function(N,K,rho,td,WL){
  # N respondents, 
  # K items
  # rho : covariance theta
  # td : optional if set time discrinination is equal to one
  library(MASS)
  ...
  sigma2 <- rlnorm(K,meanlog=0,sdlog=.3) 
  ...
  par    <-  theta[,1] %*% matrix(ab[,1],nrow=1,ncol=K) - 
				t( matrix(ab[,2],nrow=K,ncol=N))
